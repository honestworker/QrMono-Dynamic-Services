 ...
 */
function wpqrmono_plugin_path( $path = '' ) {
	return path_join( WPQRMONO_PLUGIN_DIR, trim( $path, '/' ) );
}
 ...
 * @return string URL.
 */
function wpqrmono_plugin_url( $path = '' ) {
	$url = plugins_url( $path, WPQRMONO_PLUGIN );
...
/**
 * Include a file under WPQRMONO_PLUGIN_SECTIONS_DIR.
 *
 * @param string $path File path relative to the section dir.
 * @return bool True on success, false on failure.
 */
function wpqrmono_include_section_file( $path ) {
	$dir = WPQRMONO_PLUGIN_SECTIONS_DIR;
 ...
 */
function wpqrmono_upload_dir( $type = false ) {
	$uploads = wp_get_upload_dir();

	$uploads = apply_filters( 'wpqrmono_upload_dir', array(
		'dir' => $uploads['basedir'],
		'url' => $uploads['baseurl'],
	) );
 ...
 */
function wpqrmono_verify_nonce( $nonce, $action = 'wp_rest' ) {
 ...
 */
function wpqrmono_create_nonce( $action = 'wp_rest' ) {
 ...
 */
function wpqrmono_array_flatten( $input ) {
	if ( ! is_array( $input ) ) {
		return array( $input );
	}
	...
	foreach ( $input as $value ) {
		$output = array_merge( $output, wpqrmono_array_flatten( $value ) );
	}
 ...
 */
function wpqrmono_exclude_blank( $input ) {
 ...
 */
function wpqrmono_flat_join( $input, $args = '' ) {
	$args = wp_parse_args( $args, array(
		'separator' => ', ',
	) );

	$input = wpqrmono_array_flatten( $input );
	$output = array();
 ...
 */
function wpqrmono_support_html5() {
	return (bool) wpqrmono_apply_filters_deprecated(
		'wpqrmono_support_html5',
		array( true ),
		'5.6',
		''
	);
 ...
 */
function wpqrmono_support_html5_fallback() {
	return (bool) apply_filters( 'wpqrmono_support_html5_fallback', false );
}

/**
 * Returns true if the Really Simple CAPTCHA plugin is used for forms.
 */
function wpqrmono_use_really_simple_captcha() {
	return apply_filters( 'wpqrmono_use_really_simple_captcha',
		WPQRMONO_USE_REALLY_SIMPLE_CAPTCHA
	);
}

/**
 * Returns true if wpqrmono_autop() is applied to form content.
 */
function wpqrmono_autop_or_not() {
	return (bool) apply_filters( 'wpqrmono_autop_or_not', WPQRMONO_AUTOP );
}

/**
 * Returns true if JavaScript for this plugin is loaded.
 */
function wpqrmono_load_js() {
	return apply_filters( 'wpqrmono_load_js', WPQRMONO_LOAD_JS );
}

/**
 * Returns true if CSS for this plugin is loaded.
 */
function wpqrmono_load_css() {
	return apply_filters( 'wpqrmono_load_css', WPQRMONO_LOAD_CSS );
}
 ...
 */
function wpqrmono_format_atts( $atts ) {
	$atts_filtered = array();
 ...
 */
function wpqrmono_link( $url, $anchor_text, $args = '' ) {
	$args = wp_parse_args( $args, array(
		'id' => null,
		'class' => null,
	) );
 ...
 */
function wpqrmono_get_request_uri() {
	static $request_uri = '';
 ...
 */
function wpqrmono_register_post_types() {
	if ( class_exists( 'WPQRMONO_Form' ) ) {
		WPQRMONO_Form::register_post_type();
		return true;
	} else {
		return false;
	}
 ...
 */
function wpqrmono_version( $args = '' ) {
	$defaults = array(
		'limit' => -1,
		'only_major' => false,
	);
 ...
 */
function wpqrmono_version_grep( $version, array $input ) {
 ...
 */
function wpqrmono_enctype_value( $enctype ) {
...
function wpqrmono_rmdir_p( $dir ) {
	...
	if ( $handle = opendir( $dir ) ) {
		while ( false !== ( $file = readdir( $handle ) ) ) {
			...
			wpqrmono_rmdir_p( path_join( $dir, $file ) );
		}
 ...
 */
function wpqrmono_build_query( $args, $key = '' ) {
	$sep = '&';
	$ret = array();
	...
	foreach ( (array) $args as $k => $v ) {
		...
		if ( is_array( $v ) or is_object( $v ) ) {
			array_push( $ret, wpqrmono_build_query( $v, $k ) );
		} else {
 ...
 */
function wpqrmono_count_code_units( $text ) {
	static $use_mb = null;
 ...
 */
function wpqrmono_is_localhost() {
 ...
 * @param string $version The version of Form that deprecated
 *                        the function.
 * @param string $replacement The function that should have been called.
 */
function wpqrmono_deprecated_function( $function_name, $version, $replacement ) {
	if ( WP_DEBUG ) {
		if ( function_exists( '__' ) ) {
			trigger_error(
				sprintf(
					/* translators: 1: PHP function name, 2: version number, 3: alternative function name */
					__( 'Function %1$s is <strong>deprecated</strong> since Form version %2$s! Use %3$s instead.', 'qrmono-form' ),
			...
			trigger_error(
				sprintf(
					'Function %1$s is <strong>deprecated</strong> since Form version %2$s! Use %3$s instead.',
					$function_name, $version, $replacement
				),
				E_USER_DEPRECATED
			);
 ...
 */
function wpqrmono_apply_filters_deprecated( $hook_name, $args, $version, $replacement = '' ) {
		...
		if ( $replacement ) {
			trigger_error(
				sprintf(
					...
					__( 'Hook %1$s is <strong>deprecated</strong> since Form version %2$s! Use %3$s instead.', 'qrmono-form' ),
					$hook_name,
					$version,
					$replacement
				),
			...
			trigger_error(
				sprintf(
					...
					__( 'Hook %1$s is <strong>deprecated</strong> since Form version %2$s with no alternative available.', 'qrmono-form' ),
					$hook_name,
					$version
				),
				E_USER_DEPRECATED
 ...
 * @param string $version The version of Form where the message
 *                        was added.
 */
function wpqrmono_doing_it_wrong( $function_name, $message, $version ) {
	if ( WP_DEBUG ) {
		if ( function_exists( '__' ) ) {
			if ( $version ) {
				$version = sprintf(
					/* translators: %s: Form version number. */
					__( '(This message was added in Form version %s.)', 'qrmono-form' ),
					$version
				);
			}
			...
			trigger_error(
				sprintf(
					/* translators: Developer debugging message. 1: PHP function name, 2: Explanatory message, 3: Form version number. */
					__( 'Function %1$s was called incorrectly. %2$s %3$s', 'qrmono-form' ),
					$function_name,
					$message,
					$version
				),
			...
			if ( $version ) {
				$version = sprintf(
					'(This message was added in Form version %s.)',
					$version
				);
			}
 ...
 */
function wpqrmono_log_remote_request( $url, $request, $response ) {
	$log = sprintf(
		/* translators: 1: response code, 2: message, 3: body, 4: URL */
		__( 'HTTP Response: %1$s %2$s %3$s from %4$s', 'qrmono-form' ),
	...
	$log = apply_filters( 'wpqrmono_log_remote_request',
 ...
 */
function wpqrmono_anonymize_ip_addr( $ip_addr ) {
