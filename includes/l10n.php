 ...
 */
function wpqrmono_is_valid_locale( $locale ) {
	if ( ! is_string( $locale ) ) {
		return false;
	}
 ...
 */
function wpqrmono_is_rtl( $locale = '' ) {
 ...
 */
function wpqrmono_load_textdomain( $locale = '' ) {
	$mofile = path_join(
		WP_LANG_DIR . '/plugins/',
		sprintf( '%s-%s.mo', WPQRMONO_TEXT_DOMAIN, $locale )
	);

	return load_textdomain( WPQRMONO_TEXT_DOMAIN, $mofile, $locale );
}
 ...
 * @return bool True on success, false on failure.
 */
function wpqrmono_unload_textdomain( $reloadable = false ) {
	return unload_textdomain( WPQRMONO_TEXT_DOMAIN, $reloadable );
}
 ...
 */
function wpqrmono_switch_locale( $locale, callable $callback, ...$args ) {
	...
	if ( $do_switch_locale ) {
		wpqrmono_unload_textdomain();
		switch_to_locale( $locale );
		wpqrmono_load_textdomain( $locale );
	}

	$result = call_user_func( $callback, ...$args );

	if ( $do_switch_locale ) {
		wpqrmono_unload_textdomain( true );
		restore_current_locale();
		wpqrmono_load_textdomain( $previous_locale );
	}
